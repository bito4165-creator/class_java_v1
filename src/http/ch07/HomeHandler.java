 ...
 * 3. main에서 createContext 로 경로를 짝지어 등록한다.
 */
public class HomeHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        try {
            // / <-- 이 경로를 처리하는 핸들러
            // 주의할 점
            //  "/로 등록한 핸들러는 다른 핸들러가 마지 않은 "모든"경로를 받아요
            // 그래서 정확히 "/"인지 직접 확인하고, 아니면 404을 돌려줘야 한다.

            String path = exchange.getRequestURI().getPath();

            if (!path.equals("/")) {
                SimpleHttpServer.sendResponse(exchange, 404, SimpleHttpServer.TYPE_TEXT, "404 Not Fount : " + path);
                return;
            }
            // Content-Type 을 text/html 로 보내야 브라우저가 HTML로 해석한다.
            // text/plain 으로 보이면 태그가 글자로 보인다.(확인)
            // SimpleHttpServer.sendResponse(exchange,200,"text/plain", HOME_PAGE);
            SimpleHttpServer.sendResponse(exchange,200,"text/html", HOME_PAGE);
